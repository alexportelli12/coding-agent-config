...
by calling the AI CLI as a subprocess (same auth pattern as run_eval.py —
uses the session's auth, no separate API key needed).
"""
...
def _call_ai(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    # Remove OPENCODE env var to allow nesting AI CLI calls inside a
    # session. The guard is for interactive terminal conflicts;
    # programmatic subprocess usage is safe. Same pattern as run_eval.py.
    env = {k: v for k, v in os.environ.items() if k not in ("CLAUDECODE", "OPENCODE")}
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"AI CLI exited {result.returncode}\nstderr: {result.stderr}"
        )
...
def improve_description(
    ...
    prompt = f"""You are optimizing a skill description for an AI coding tool skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there's a title and description that the AI sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # The old SDK path did this as a true multi-turn; the CLI tool is one-shot, so we
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_ai(shorten_prompt, model)
        match = re.search(
